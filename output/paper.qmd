...
author: "Boxuan Yi"
format: pdf
abstract: "Hate crime data provide crucial insights into underlying biases and social dynamics. This paper uses data from Open Data Toronto to analyze the distribution of hate crimes from 2018 to 2023, focusing on biases and locations. The findings reveal that Jewish people are the most frequently targeted, with incidents continuing to rise, followed by Black individuals. Additionally, nearly half of the crimes occurred in public spaces or transportation."
...
```
The issue of hate crimes is a global concern. Hate crimes are typically defined as criminal offences committed against a person or property based solely on the victim’s identity, such as race, religion, sexual orientation, gender, or disability [@justice]. Beyond the harm they inflict on individual victims, hate crimes foster fear and division within society. As one of the world’s most diverse cities, Toronto has experienced a rising number of hate crimes, which prompts widespread public concern. This trend highlights the urgent need for effective interventions that can address the root causes of such crimes. To better reduce hate incidents and develop strategies, it is important to analyze hate crime data — specifically, whether criminals are targeting particular groups and, if so, which ones.

This paper utilizes data from Open Data Toronto to explore the distribution of hate crimes by bias and location from 2018 to 2023. The biases examined include race, religion, and mental or physical disability. The locations considered are various regions in Toronto and different types of settings where crimes occur. The results show that a significant proportion of hate crimes were directed at Black individuals and Jewish people, accounting for approximately 16% and 31% of incidents, respectively. About half of the crimes occurred in public spaces or transportation, and about half were property crimes, with increasing trends observed in educational and commercial/business settings. 
...
A hate crime is a criminal offence committed against a person motivated by bias and prejudice [@hatecrime]. This analysis uses hate crime data from Open Data Toronto, covering verified hate crime occurrences from 2018 to 2023, updated annually. The dataset categorizes crimes by various bias types, such as age, disability, race, ethnicity, language, religion, sexual orientation, and gender. Provided by the Toronto Police Service, this dataset aims to enhance community awareness and public safety [@police]. 

The variables or measurements included in the dataset are ID, occurrence time (time, date and year), reported time (time, date and year), police division, location type, various bias categories (age, disability, race, ethnicity, language, religion, sexual orientation, and gender), the primary offence related to the hate crime, neighbourhood, and arrest status. I also created two new variables: `occurrence_month`, derived from the occurrence date, and `division_area`, which groups the 17 police divisions, including 'NSA', into 5 regions of Toronto, with an additional 'Other' category. The data is provided at the occurrence level. The location of Hate Crime occurrences have been grouped to the neighbourhood level to protect the privacy of parties and communities involved in the occurrence. 

The raw dataset contains 1350 hate crimes. After cleaning, 1287 rows remain. This analysis mainly focuses on biases related to disability, race, and religion, as well as the time and location of hate crimes. 

## Data Cleaning
Missing values have been removed from the dataset. A new column `occurrence_month` has been created by extracting the month from the `occurrence_date`. 

The `religion_bias` column has been cleaned by consolidating various Christian denominations into a single ‘Christian’ category, while mixed biases have been excluded. Similarly, the unclear values in the `race_bias` column, such as ‘Black, South Asian’, have been removed. The `location_type` column has been regrouped to simplify the categorization. Additionally, a new column `primary_offence_simplified` has been mutated to group primary offenses into major categories, such as ‘Assault’ and ‘Property Crimes’. Finally, the dataset has been modified to include a simplified `division_area` column based on police divisions.
